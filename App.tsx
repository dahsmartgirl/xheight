...
import { Download, X, CheckCircle2, FileArchive, ChevronLeft, ChevronRight, Menu, RotateCcw, Sun, Moon, Monitor } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setEffectiveTheme(activeTheme as 'light' | 'dark');
  ...
  return (
    <div className="w-full h-[100dvh] bg-white dark:bg-neutral-950 flex flex-col font-['Inter'] overflow-hidden transition-colors duration-200">
      
      {/* Header */}
      <header className="h-[54px] shrink-0 border-b border-thin border-[#D9D9D9] dark:border-neutral-800 flex items-center justify-between px-4 lg:px-[24px] bg-white dark:bg-neutral-950 z-50 relative overflow-visible transition-colors duration-200">
        ...
        <div className="flex items-center gap-2 lg:gap-3">
            ...
            <div className="relative">
                <button 
                  ...
                  className="flex items-center justify-center w-8 h-8 lg:w-9 lg:h-9 rounded-full bg-gray-100 dark:bg-neutral-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-neutral-700 transition-colors"
                >
                ...
                {isThemeMenuOpen && (
                  <>
                    ...
                    <div className="absolute top-full right-0 mt-2 w-32 bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-lg shadow-lg py-1 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                        <button 
                          onClick={() => { setTheme('light'); setIsThemeMenuOpen(false); }}
                          className={`w-full text-left px-3 py-2 text-sm flex items-center gap-2 ${theme === 'light' ? 'bg-gray-50 dark:bg-neutral-800 text-black dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-neutral-800'}`}
                        >
                          <Sun size={14} /> Light
                        </button>
                        <button 
                          onClick={() => { setTheme('dark'); setIsThemeMenuOpen(false); }}
                          className={`w-full text-left px-3 py-2 text-sm flex items-center gap-2 ${theme === 'dark' ? 'bg-gray-50 dark:bg-neutral-800 text-black dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-neutral-800'}`}
                        >
                          <Moon size={14} /> Dark
                        </button>
                        <button 
                          onClick={() => { setTheme('system'); setIsThemeMenuOpen(false); }}
                          className={`w-full text-left px-3 py-2 text-sm flex items-center gap-2 ${theme === 'system' ? 'bg-gray-50 dark:bg-neutral-800 text-black dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-neutral-800'}`}
                        >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {/* Main Content Area */}
        <main className="flex-1 flex flex-col gap-[16px] lg:gap-[24px] p-4 lg:p-[24px] lg:pl-[32px] bg-white dark:bg-neutral-950 overflow-hidden w-full transition-colors duration-200">
             
             {/* Top Control Bar */}
             <div className="flex flex-row items-center justify-between w-full gap-3 shrink-0">
                 
                 {/* Canvas/Preview Toggle Pill */}
                 <div className="flex-1 sm:flex-none sm:w-[220px] h-[40px] p-[3px] bg-[#FAFAFA] dark:bg-neutral-900 rounded-[30px] flex relative shrink-0 transition-colors">
                      <button 
                      ...
                      <div className="flex items-center gap-3 pr-2 pl-1.5 py-1.5 rounded-full border border-transparent transition-all">
                           <div className="relative w-[28px] h-[28px] rounded-full flex items-center justify-center shadow-inner bg-white dark:bg-neutral-800">
                               <div 
                                 ...
                                 style={{
                                     background: `conic-gradient(#ED0C14 ${progressPercent}%, ${effectiveTheme === 'dark' ? '#262626' : '#F2F2F2'} ${progressPercent}% 100%)`
                                 }}
                               />
                               <div className="absolute inset-[3px] bg-white dark:bg-neutral-900 rounded-full flex items-center justify-center transition-colors">
